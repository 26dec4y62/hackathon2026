...
function mercatorProject(lat, lng, canvasW, canvasH) {
...
function getLondonBounds() {
  ...
  Object.values(POSTCODE_COORDS_MAP).forEach(([lat, lng]) => {
...
// Project postcode to canvas coordinates
function getPostcodeCoords(code, canvasW, canvasH) {
  if (POSTCODE_COORDS_MAP[code]) {
    const [lat, lng] = POSTCODE_COORDS_MAP[code];
    const minProj = mercatorProject(LONDON_BOUNDS.minLat, LONDON_BOUNDS.minLng, 1000, 1000);
    const maxProj = mercatorProject(LONDON_BOUNDS.maxLat, LONDON_BOUNDS.maxLng, 1000, 1000);
    const coordProj = mercatorProject(lat, lng, 1000, 1000);
    const x = ((coordProj[0] - minProj[0]) / (maxProj[0] - minProj[0])) * canvasW;
    const y = ((coordProj[1] - minProj[1]) / (maxProj[1] - minProj[1])) * canvasH;
    return [x, y];
  }
...
function HeatmapPage() {
  ...
  const focused = (hovered && hovered.code === selectedCode)
    ? hovered
    : (hovered || ALL_POSTCODE_DATA.find(p => p.code === selectedCode) || null);
  ...
  useEffect(() => {
    ...
    ALL_POSTCODE_DATA.forEach(p => {
      ...
      const dotR = isHovered ? 10 : (isSelected ? 8 : 6.5);
      ...
      // bolder border on every postcode marker
      ctx.beginPath();
      ctx.arc(cx, cy, dotR, 0, Math.PI * 2);
      ctx.strokeStyle = "rgba(44,41,36,0.55)";
      ctx.lineWidth = 1.8;
      ctx.stroke();

      if (isHovered || isSelected) {
        // stronger outer highlight ring
        ctx.beginPath();
        ctx.arc(cx, cy, dotR + 4.5, 0, Math.PI * 2);
        ctx.strokeStyle = scoreToColor(score, 0.75);
        ctx.lineWidth = isHovered ? 3.5 : 2.5;
        ctx.stroke();
      }

      ctx.font = (isHovered || isSelected) ? "500 12px 'DM Sans', sans-serif" : "300 11px 'DM Sans', sans-serif";
      ctx.fillStyle = (isHovered || isSelected) ? "#2C2924" : "#A09890";
  ...
  }, [activeFactor, hovered, selectedCode]);

  function getClosestPostcode(mx, my, threshold = 30) {
    let closest = null;
  ...
  function handleMouseMove(e) {
    ...
    setHovered(getClosestPostcode(mx, my));
  }
  ...
  function handleCanvasClick(e) {
    ...
    const mx = (e.clientX - rect.left) * scaleX;
    const my = (e.clientY - rect.top) * scaleY;
    const clicked = getClosestPostcode(mx, my);
    setSelectedCode(clicked ? clicked.code : null);
  }

  function handleCanvasWheel(e) {
    // Prevent browser pinch-zoom while adjusting controls near the canvas.
          ...
          <div className="heatmap-tooltip-section">
            {!focused ? (
              <div className="heatmap-tooltip-empty">
                Hover or click a postcode on the map to see its score
              </div>
            ) : (
              <div style={{animation:"fadeUp 0.2s ease both"}}>
                <div className="heatmap-tooltip-code">{focused.code}</div>
                <div className="heatmap-tooltip-area">{focused.area}</div>
                <div className="heatmap-tooltip-score-row">
                  <div className="heatmap-tooltip-score" style={{color: scoreToColor(focused[activeFactor], 1)}}>
                    {focused[activeFactor]}
                  </div>
                  <div className="heatmap-tooltip-score-label">/ 100<br/>{factor.label.toLowerCase()}</div>
                </div>
                <div className="heatmap-tooltip-verdict">{getVerdict(focused[activeFactor])}</div>
                    ...
                    {FACTORS.map(f => (
                      <div className="heatmap-mini-row" key={f.id}>
                        ...
                        <div className="heatmap-mini-track">
                          <div className="heatmap-mini-fill" style={{width:`${focused[f.id]}%`, background: f.color}}></div>
                        </div>
                        <div className="heatmap-mini-val">{focused[f.id]}</div>
                      </div>
                    ))}
                  </div>
                </div>
